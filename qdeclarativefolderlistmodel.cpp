    ...
    : QAbstractListModel(parent)
{
    ...
    roles[FilePathRole] = "fsPath";
    roles[IS_Folder] = "isFolder";
    setRoleNames(roles);
...
void QDeclarativeFolderListModel::setFolder(const QUrl &folder)
{
    if (folder == d->folder)
        return;
    if(folder.isParentOf(m_rootFolder))
        return;
    ...
    if(m_onRoot != (d->folder == m_rootFolder)){
        m_onRoot ^=1;
        emit onRootChanged(m_onRoot);
//            qDebug() << "onRoot Changed to: " << m_onRoot;
    }
}
...
bool QDeclarativeFolderListModel::onRoot() const
{
    return m_onRoot;
}
/*!
...
bool QDeclarativeFolderListModel::isFolder(int index) const
{
    if (index != -1) {
        ...
        if (idx.isValid()){
            return d->model.isDir(idx);
//            QString path = d->model.data(idx,QDirModel::FilePathRole).toString();
//            qDebug() << "QDeclarativeFolderListModel::isFolder, file path: " << path;
...
void QDeclarativeFolderListModel::refresh()
{
    ...
    QModelIndex index = d->model.index(d->folder.toLocalFile());
...
void QDeclarativeFolderListModel::setRootFolder(QUrl arg)
{
    if (m_rootFolder != arg) {
        m_rootFolder = arg;
        emit rootFolderChanged(arg);
